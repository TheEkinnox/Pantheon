#pragma once
#include "PantheonRendering/Geometry/BoundingBox.h"
#include "PantheonRendering/Geometry/BoundingSphere.h"

namespace PantheonRendering::Geometry
{
    class Frustum
    {
    public:
        enum EPlane : uint8_t
        {
            PLANE_LEFT = 0,
            PLANE_RIGHT,
            PLANE_TOP,
            PLANE_BOTTOM,
            PLANE_NEAR,
            PLANE_FAR,
            PLANE_COUNT
        };

        /**
         * \brief Computes the frustum from the given view-projection matrix
         * \param viewProjection The source view-projection matrix
         */
        explicit Frustum(const LibMath::Matrix4& viewProjection);

        /**
         * \brief Creates a copy of the given frustum
         * \param other The copied frustum
         */
        Frustum(const Frustum& other) = default;

        /**
         * \brief Creates a move copy of the given frustum
         * \param other The moved frustum
         */
        Frustum(Frustum&& other) noexcept = default;

        /**
         * \brief Destroys the frustum
         */
        ~Frustum() = default;

        /**
         * \brief Assigns a copy of the given frustum to this one
         * \param other The copied frustum
         * \return The modified frustum
         */
        Frustum& operator=(const Frustum& other) = default;

        /**
         * \brief Moves the given frustum into this one
         * \param other The moved frustum
         * \return The modified frustum
         */
        Frustum& operator=(Frustum&& other) noexcept = default;

        /**
         * \brief Checks if a given point is in the camera's frustum
         * \param point The point to check
         * \return True if the point is in the camera's frustum. False otherwise
         */
        bool intersects(const LibMath::Vector3& point) const;

        /**
         * \brief Checks if a given bounding sphere intersects the camera's frustum
         * \param boundingSphere The target bounding sphere
         * \return True if the sphere is in the camera's frustum. False otherwise
         */
        bool intersects(const BoundingSphere& boundingSphere) const;

        /**
         * \brief Checks if a given bounding box is in the camera's frustum
         * \param boundingBox The target bounding box
         * \return True if the bounding box is in the camera's frustum. False otherwise
         */
        bool intersects(const BoundingBox& boundingBox) const;

    private:
        LibMath::Vector4 m_planes[PLANE_COUNT];
    };
}
